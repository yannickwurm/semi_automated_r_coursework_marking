...
We ran each of the tests outlined above. In the output below, a first "table" shows R's interactive output while it ran the tests, where it counts how many failed an how many passed.

The second section shows the outputs of the tests that were unsuccessful. R indicates `-- Failure` for each of these. Here, you can see:

  * the description of the test that was run (e.g., "Remove all of the following words: 'the', 'really', 'truly', 'very'.")
  * the line of code that was run (e.g., `shorten_text("This is the really truly very best test.")`), R's explanation of the problem (e.g., `not equal to`) and the expected output.
  * It then provides more details on the comparison it did. When comparing text, x[1] indicates what your code output, and [1] indicates what would have been expected.
